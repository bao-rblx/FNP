...
export default function Auth() {
  ...
  const onSignup = async (e: FormEvent) => {
    e.preventDefault();
    if (!suName.trim() || !suContact.trim() || !suPassword || !suConfirm) {
      toast.error(t.authFillFields);
      return;
    }
    ...
    if (!isValidEmail(contact)) {
      toast.error(language === 'en' ? 'Please enter a valid email' : 'Vui lòng nhập email hợp lệ');
      return;
    }
    ...
    setBusy(true);
    try {
      const r = await signUp(suName, contact, suPassword);
      if (!r.ok) {
      ...
      return (
        <form onSubmit={onForgot} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="forgot-email">{language === 'vi' ? 'Email' : 'Email'}</Label>
            <Input id="forgot-email" type="email" value={resetEmail} onChange={(e) => setResetEmail(e.target.value)} placeholder="email@example.com" />
          </div>
          <Button type="submit" disabled={busy} className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold">{t.sendResetLink}</Button>
    ...
    if (authMode === 'reset') {
      return (
        <form onSubmit={onReset} className="space-y-4">
          ...
          <Button type="submit" disabled={busy} className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold">{t.resetPassword}</Button>
        </form>
      );
    }
      ...
      <Tabs value={authMode as any} onValueChange={(v) => setAuthMode(v as any)} className="w-full">
        ...
        <TabsContent value="login" className="space-y-4">
          <form onSubmit={onLogin} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="login-email" type="email" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} placeholder="email@example.com" />
            </div>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label htmlFor="login-password">{t.password}</Label>
                <button type="button" onClick={() => setAuthMode('forgot')} className="text-xs text-indigo-500 hover:underline">{t.forgotPassword}</button>
              </div>
            ...
            <Button type="submit" disabled={busy || isSuccess} className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold shadow-lg shadow-indigo-600/20">{t.signInButton}</Button>
            <p className="text-center text-sm text-muted-foreground">
              {t.authNoAccount} <button type="button" className="text-indigo-500 font-medium hover:underline" onClick={() => setAuthMode('signup')}>{t.signupTitle}</button>
            </p>
          </form>
        </TabsContent>
        ...
        <TabsContent value="signup" className="space-y-4">
          <form onSubmit={onSignup} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="su-name">{t.username}</Label>
              <Input id="su-name" value={suName} onChange={(e) => setSuName(e.target.value)} placeholder={language === 'en' ? 'e.g. polymaster' : 'vd: polymaster'} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="su-contact">Email</Label>
              <Input id="su-contact" type="email" value={suContact} onChange={(e) => setSuContact(e.target.value)} placeholder="email@example.com" />
            </div>
            ...
            <Button type="submit" disabled={busy || isSuccess} className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold shadow-lg shadow-indigo-600/20">{t.signUpButton}</Button>
            <p className="text-center text-sm text-muted-foreground">
              {t.authHasAccount} <button type="button" className="text-indigo-500 font-medium hover:underline" onClick={() => setAuthMode('login')}>{t.loginTitle}</button>
            </p>
          </form>
        </TabsContent>
      </Tabs>
      ...
      <div className="min-h-screen bg-muted pb-20 md:pb-8 md:pt-16">
        ...
        <div className="max-w-md mx-auto px-4 py-6">
          <Card className="border-border shadow-sm">
            <CardHeader>
              <CardTitle className="text-xl">PolyStore</CardTitle>
              <CardDescription>
