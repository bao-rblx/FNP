...
const CATEGORY_KEYS = ['all', 'models'] as const;
...
export default function Services() {
  ...
  const categoryLabels = React.useMemo(() => {
    return {
      ...
      models: language === 'vi' ? 'Mô hình 3D' : '3D Models',
    };
  }, [language]);
            ...
            >
              {categoryProducts.map((product) => (
                <motion.div
                  ...
                  whileHover={{ y: -4 }}
                  className="bg-card rounded-2xl overflow-hidden border border-border shadow-sm shadow-black/5 hover:shadow-xl hover:shadow-black/10 hover:border-primary/30 transition-all duration-300 flex flex-col group"
                >
                  <Link to={`/product/${product.id}`} className="block h-full flex flex-col">
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      <h3 className="font-medium text-sm md:text-base mb-1.5 line-clamp-2 text-card-foreground group-hover:text-primary transition-colors">
                        {productName(product)}
                      </h3>
